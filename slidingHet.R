######  Data load #####
#data <- read.csv(dataset, header = T,stringsAsFactors = F)
...
linearCoordinates <<- function(data, transformAxis = "Longitude") {
  ...
  if (tolower(transformAxis) == "longitude") {
      tmpData$transformed<-(LATITUDE-intercept)/slope
      return (list(tmpData,wVal))
  } 
  if (tolower(transformAxis) == "latitude") {
    tmpData$transformed <- slope*LONGITUDE+intercept
    return (list(tmpData,wVal))
  }
  else{
